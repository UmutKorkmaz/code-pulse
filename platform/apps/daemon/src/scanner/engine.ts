...
import type { AiSessionRow, DatabaseV5 } from '@codepulse/core';
import {
    LocalRegistry,
    RegistryHost,
...
import { createDaemonEnvelope, deriveEnvelopeId } from '../envelope';
import { LogTailer, type LogTailBatch, type TailedLogEvent } from './log-tailer';
import {
    ProcessWatcher,
    type ProcessDetection,
    type ProcessSessionEvent
} from './process-watcher';

/**
 * `ingestLogBatchWithCursor` is implemented by DatabaseV5 in
 * `@codepulse/core` source; this augmentation merges the signature so the
 * daemon typechecks against the new API before the package's emitted
 * declarations are rebuilt.
 */
declare module '@codepulse/core' {
    interface DatabaseV5 {
        ingestLogBatchWithCursor(
            envelopes: AnyEnvelope[],
            cursor: LogTailBatch['cursor']
        ): Promise<AnyEnvelope[]>;
    }
}
...
export class ScannerEngine extends EventEmitter {
    ...
    private readonly registryHost: RegistryHost;
    private readonly processWatcher: ProcessWatcher;
    private readonly logTailer: LogTailer;
    private manifests: ScannerManifest[] = [];
    private quarantined = new Set<string>();
    private started = false;
    private ingestChain: Promise<void> = Promise.resolve();

    constructor(private readonly deps: ScannerEngineDeps) {
        super();
        this.registry = new LocalRegistry(deps.registryDir);
        this.registryHost = new RegistryHost(this.registry);

        const isEnabled = (manifest: ScannerManifest) => this.isScannerEnabled(manifest);
        ...
        this.processWatcher.on('session-started', event => {
            this.enqueueIngest(() =>
                this.handleProcessSessionStarted(event as ProcessSessionEvent)
            );
        });

        this.processWatcher.on('session-ended', event => {
            this.enqueueIngest(() => this.handleProcessSessionEnded(event as ProcessSessionEvent));
        });

        this.logTailer.on('batch', batch => {
            this.enqueueIngest(() => this.handleParsedLogBatch(batch as LogTailBatch));
        });
    ...
    getManifests(): ScannerManifest[] {
        return [...this.manifests];
    }

    private isScannerEnabled(manifest: ScannerManifest): boolean {
    ...
    private async syncRegistryRows(): Promise<void> {
    ...
    private async handleProcessDetection(detection: ProcessDetection): Promise<void> {
        const envelope = createDaemonEnvelope({
        ...
        await this.deps.database.touchRegistryScannerLastScan(
            detection.scannerId,
     ...
     */
    private async handleProcessSessionStarted(event: ProcessSessionEvent): Promise<void> {
        const envelope = createDaemonEnvelope({
     ...
     */
    private async handleProcessSessionEnded(event: ProcessSessionEvent): Promise<void> {
    ...
    private async findOpenAiSession(scannerId: string, tool: string): Promise<AiSessionRow | null> {
        const sessions = await this.deps.database.listAiSessions(1000);
        return (
            sessions.find(
                row => row.scanner_id === scannerId && row.tool === tool && row.ended_at === null
            ) ?? null
        );
    }
     ...
     * envelope ids.
     */
    private async handleParsedLogBatch(batch: LogTailBatch): Promise<void> {
    ...
    private buildLogEventEnvelopes(batch: LogTailBatch, event: TailedLogEvent): AnyEnvelope[] {
        ...
        const idFor = (envelopeType: string): string =>
            deriveEnvelopeId(
                manifest.id,
                batch.filePath,
                String(event.byteOffset),
                event.lineHash,
                envelopeType
            );
    ...
    private async publishEnvelope(envelope: AnyEnvelope, source: 'scanner'): Promise<boolean> {
        const framed: AnyEnvelope = { ...envelope, src: source };
...
function isSessionStartEvent(eventType?: string): boolean {
    if (!eventType) {
        return false;
    }
...
function isSessionEndEvent(eventType?: string): boolean {
    if (!eventType) {
        return false;
    }
    const normalized = eventType.toLowerCase();
