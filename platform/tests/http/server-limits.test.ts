...
): Promise<{ response?: HttpResponse; bytesWritten: number; aborted: boolean }> {
        ...
        const req = http.request(
            ...
            res => {
                ...
                res.on('end', () => {
                    ...
                    finish({
                        response: { status: res.statusCode ?? 0, body, raw } as HttpResponse,
                        bytesWritten,
                        aborted
                    });
                });
...
describe('daemon http request limits', () => {
    ...
    it('rejects an oversized POST body with 413 without buffering it all', async () => {
        ...
        const { response, bytesWritten, aborted } = await postStream(
            port,
            '/v1/events/ingest',
            oversized
        );
        ...
        assert.ok(
            bytesWritten < oversized,
            `expected the daemon to abort before ${oversized} bytes, wrote ${bytesWritten}`
        );
        ...
        assert.ok(
            bytesWritten <= MAX_REQUEST_BODY_BYTES + 4 * 1024 * 1024,
            `expected the writer to stall near the ${MAX_REQUEST_BODY_BYTES}-byte cap, wrote ${bytesWritten}`
        );
    });
    ...
    it('clamps /v1/ai/tokens days to 90 so it cannot fan out unbounded queries', async () => {
        ...
        assert.strictEqual(aggregateCalls, AI_TOKENS_MAX_DAYS);
    });
});
